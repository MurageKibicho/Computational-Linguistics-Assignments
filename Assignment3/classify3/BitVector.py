...
class BitVector( object ):                                           #(A1)
    ...
    def __init__( self, *args, **kwargs ):                           #(A2)
        ...
        if 'filename' in kwargs:filename=kwargs.pop('filename')#(A9)
        if 'fp' in kwargs:           fp = kwargs.pop('fp')    #(A10)
        if 'size' in kwargs:       size = kwargs.pop('size')  #(A11)
        if 'intVal' in kwargs:   intVal = kwargs.pop('intVal')#(A12)
            ...
            if filename or fp or size or intVal or bitlist:         #(A73)
                raise ValueError(                                   #(A74)
                  '''When a bitstring is specified, you cannot
                     give values to any other constructor args''')
            bitlist =  list(map( int, list(bitstring) ))            #(A75)
